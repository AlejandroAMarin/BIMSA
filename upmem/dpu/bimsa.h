...
void opt3_umem_wfa_extend(int k_min, int k_max,
	...
	printf("[DEBUG] EXTEND STARTED plen %d tlen %d pstart %d tstart %d\n", pattern_length, text_length, ma_pattern_start, ma_text_start);
	...
	for (k = k_min; k <= k_max; ++k)
	{
		...
		int acc = 0;
		const uint32_t invert_mask = 0x01010101;
		...
		while (v < pattern_length && h < text_length) {			
			...
			printf("[DEBUG] alingment p %d alingment t %d \n", read_idx_patt_char, read_idx_text_char);
			...
			const uint32_t cmp_result = cmp4out ^ invert_mask;
			printf("[DEBUG] pattern vs text \n");
			printf("%s\n%s\n", &cache_pattern[read_idx_patt_char], &cache_text[read_idx_text_char]);
			for (int i = (sizeof(uint32_t) * 8) - 1; i >= 0; i--) {
				printf("%d", (cmp_result >> i) & 1);
				if (i % 8 == 0)
					printf(" ");
			}
			printf("\n");

			int32_t eq_elements = __builtin_ctzl(cmp_result) / 8;

			if (eq_elements >= 4){
				// printf("[DEBUG] COUNT LEADING ZEROS %d\n", eq_elements);
				eq_elements = 4;
			}		
			printf("[DEBUG] COUNT LEADING ZEROS %d\n", eq_elements);

			// Check pattern and text boundaries and count the zeros to know the mismatching offset
			const int32_t next_v = v + bases_to_cmp;
			const int32_t next_h = h + bases_to_cmp;
			int32_t max_pattern_values = pattern_length - v;
			int32_t max_text_values = text_length - h;
		...
		printf("[DEBUG] matching characters %d\n", acc);
	...
	mram_write(cache_wavefront_offsets,(__mram_ptr void *) cma_wf_offsets, WF_TRANSFER);
	printf("[DEBUG] EXTEND FINISHED SUCCESFULLY\n\n");
	#if PERF_SECTIONS
...
ewf_offset_t umem_wfa_compute_and_extend(int32_t lo, int32_t hi,
	...
	int32_t pattern_length, int32_t text_length, 
	uint32_t ma_pattern_start, uint32_t ma_text_start, 
	char* cache_pattern, char* cache_text, uint16_t wf_transfer_size){
	//profiling_start(&compute);
	#if PERF_SECTIONS
	perfcounter_cycles counter;
	timer_start(&counter);
...
int base_wavefront(uint32_t ma_pattern_start, uint32_t ma_text_start, 
		...
		uint32_t* cma_cigar_aux,
		int* cigar_aux_pos, const uint32_t base_limit
		){
...
int find_breakpoint_iterative(int32_t pattern_length, int32_t text_length,
	...
	// extend forward
	opt3_umem_wfa_extend(wf_fw_lo, wf_fw_hi,
	...
	while(1)
	{
		...
		max_fw = umem_wfa_compute_and_extend(wf_fw_lo, wf_fw_hi,
				...
				pattern_length, text_length, 
				ma_pattern_start, ma_text_start, 
				cache_pattern, cache_text, wffw_transfer_size);
		#if PERF_MAIN
		result->main += timer_stop(main_counter);
		timer_start(main_counter);
		#endif
		...
		if(overlap) {
			if(distance_fw + distance_rv + 4 >= threshold){
				...
				base_wavefront(ma_pattern_start, ma_text_start, 
										...
										cache_cigar_aux,
										cma_cigar_aux, cigar_aux_pos, base_limit);
				#if PERF_MAIN
				result->main += timer_stop(main_counter);
				timer_start(main_counter);
				#endif
		...
		max_rv = umem_wfa_compute_and_extend_rv(wf_rv_lo, wf_rv_hi,
				...
				pattern_length, text_length, 
				ma_pattern_start, ma_text_start, 
				cache_pattern, cache_text, wfrv_transfer_size);
		#if PERF_MAIN
		result->main += timer_stop(main_counter);
		timer_start(main_counter);
		#endif
